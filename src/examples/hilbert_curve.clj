...
(defn regenerate
  ...
  [{:keys [order size animate?]
    :as state} prev-order prev-size]
...
(defn draw [{:keys [path counter thick order size animate?]
             :as state}]
  (rcd/begin-drawing!)
  ...
  (let [animate? (gui/check-box {:x 450.0
                                 :y 50.0
                                 :width 20.0
                                 :height 20.0}
                                "ANIMATE GENERATION ON CHANGE" animate?)
        order' (gui/spinner {:x 585.0
                             :y 100.0
                             :width 180.0
                             :height 30.0}
                            "HILBERT CURVE ORDER:  " order 2 8)
        thick (gui/slider {:x 524.0
                           :y 150.0
                           :width 240.0
                           :height 24.0}
                          "THICKNESS:  " nil thick 1.0 10.0)
        size' (gui/slider {:x 524.0
                           :y 190.0
                           :width 240.0
                           :height 24.0}

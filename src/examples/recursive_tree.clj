...
(defn branches
  ...
  (let [theta (Math/toRadians angle-degrees)
        ...
        root {:start start
              :end (tip start 0.0 length)
              :angle 0.0
              :length length}]
    (loop [acc [root] i 0]
      (if (>= i (count acc))
        acc
        (let [{:keys [end angle length]
               :as b} (nth acc i)
          ...
          (if (or (< (:length b) min-branch-length)
            ...
            (let [a1 (+ angle theta) a2 (- angle theta)]
              (recur (conj acc
                           {:start end
                            :end (tip end a1 next-length)
                            :angle a1
                            :length next-length}
                           {:start end
                            :end (tip end a2 next-length)
                            :angle a2
                            :length next-length})
                     (inc i)))))))))
...
(defn draw [{:keys [angle thick tree-depth branch-decay length bezier?]
             :as state}]
  (rcd/begin-drawing!)
  (rcd/clear-background! colors/raywhite)

  (let [start {:x (- (/ screen-width 2.0) 125.0)
  ...
  (let [bar (fn [y label fmt v mn mx]
        ...
        bezier? (gui/check-box {:x 640.0
                                :y 190.0
                                :width 20.0
                                :height 20.0} "Bezier" bezier?)]
    (rtd/draw-fps! 10 10)
    (debug-stats/draw!)
    (rcd/end-drawing!)
